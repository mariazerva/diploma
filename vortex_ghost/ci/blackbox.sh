...
show_usage()
{
    ...
    echo "Usage: $0 [[--clusters=#n] [--cores=#n] [--warps=#n] [--threads=#n] [--issue_width=#n] [--isb=#n] [--infl=#n] [--l2cache] [--l3cache] [--driver=#name] [--app=#app] [--args=#args] [--debug=#level] [--scope] [--perf=#class] [--rebuild=#n] [--log=logfile] [--help]]"
}
...
# default isb instructions
if [ $ISB_INSTRS -eq -1 ]; then
    ISB_INSTRS=8
fi

# default infl instructions
if [ $INFL_INSTRS -eq -1 ]; then
    INFL_INSTRS=8
fi
...
# set configurations
CONFIGS="-DNUM_CLUSTERS=$CLUSTERS -DNUM_CORES=$CORES -DNUM_WARPS=$WARPS -DNUM_THREADS=$THREADS -DISSUE_WIDTH=$ISSUE_WIDTH -DISB_INSTRS=$ISB_INSTRS -DINFL_INSTRS=$INFL_INSTRS $L2 $L3 $PERF_FLAG $CONFIGS"
